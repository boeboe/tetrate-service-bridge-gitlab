...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for TSB tenant investmentbank to be configured correctly (pipeline applications/investmentbank/tsb-tenant-admin)"
  while true; do
    status_investmentbank_tenant=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/investmentbank" "tsb-tenant-admin")
    if [[ ${status_investmentbank_tenant} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
