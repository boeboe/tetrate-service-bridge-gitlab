...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for TSB tenant privatebank to be configured correctly (pipeline applications/privatebank/tsb-tenant-admin)"
  while true; do
    status_privatebank_tenant=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/privatebank" "tsb-tenant-admin")
    if [[ ${status_privatebank_tenant} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
