...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for TSB tenant cashmgmt to be configured correctly (pipeline applications/cashmgmt/tsb-tenant-admin)"
  while true; do
    status_cashmgmt_tenant=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/cashmgmt" "tsb-tenant-admin")
    if [[ ${status_cashmgmt_tenant} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
