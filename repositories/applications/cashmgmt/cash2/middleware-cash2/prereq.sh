...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for tenant namespaces to be configured correctly (pipeline applications/cashmgmt/tsb-tenant-admin)"
  while true; do
    status_tenant_namespaces=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/cashmgmt" "tsb-tenant-admin")
