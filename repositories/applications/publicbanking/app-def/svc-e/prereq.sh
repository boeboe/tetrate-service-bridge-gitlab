...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for TSB workspace app-def to be configured correctly (pipeline applications/publicbanking/app-def/workspace-admin)"
  while true; do
    status_appdef_workspace=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/publicbanking/app-def" "workspace-admin")
    if [[ ${status_appdef_workspace} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
