...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for TSB tenant wealthmgmt to be configured correctly (pipeline applications/wealthmgmt/tsb-tenant-admin)"
  while true; do
    status_wealthmgmt_tenant=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/wealthmgmt" "tsb-tenant-admin")
    if [[ ${status_wealthmgmt_tenant} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
