...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for TSB workspace app-uvw to be configured correctly (pipeline applications/privatebanking/app-uvw/workspace-admin)"
  while true; do
    status_appuvw_workspace=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/privatebanking/app-uvw" "workspace-admin")
    if [[ ${status_appuvw_workspace} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
