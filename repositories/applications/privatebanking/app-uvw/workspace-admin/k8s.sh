...
if [[ ${ACTION} = "deploy" ]]; then
  ...
  if kubectl --context mgmt -n tier1-gw-uvw get secret app-uvw-cert &>/dev/null; then
    echo "Secret 'app-uvw-cert' in namespace 'tier1-gw-uvw' already exists in cluster 'mgmt'"
  else
    echo "Creating secret 'app-uvw-cert' in namespace 'tier1-gw-uvw' in cluster 'mgmt'"
    kubectl --context mgmt create secret generic app-uvw-cert -n tier1-gw-uvw \
  ...
  for cluster_name in active standby ; do
    if kubectl --context ${cluster_name} -n gateway-uvw get secret app-uvw-cert &>/dev/null; then
      echo "Secret 'app-uvw-cert' in namespace 'gateway-uvw' already exists in cluster '${cluster_name}'"
    else
      echo "Creating secret 'app-uvw-cert' in namespace 'gateway-uvw' in cluster '${cluster_name}'"
      kubectl --context ${cluster_name} create secret tls app-uvw-cert -n gateway-uvw \
