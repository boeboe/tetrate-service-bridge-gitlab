...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for TSB tenant privatebanking to be configured correctly (pipeline applications/privatebanking/tenant-admin)"
  while true; do
    status_privatebanking_tenant=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "applications/privatebanking" "tenant-admin")
    if [[ ${status_privatebanking_tenant} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
