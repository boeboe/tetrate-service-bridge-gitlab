...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for LDAP to be synchronized correctly (pipeline platform/ldap-users)"
  while true; do
    status_platform_ldap_users=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "platform" "ldap-users")
    if [[ ${status_platform_ldap_users} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
