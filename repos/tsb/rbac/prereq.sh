...
if [[ ${ACTION} = "check" ]]; then
  ...
  print_info "Wait for LDAP to be synchronized correctly (pipeline infra/ldap)"
  while true; do
    status_infra_ldap=$(gitlab_get_pipeline_status ${CI_API_V4_URL} "01234567890123456789" "infra" "ldap")
    if [[ ${status_infra_ldap} == "success" ]] ; then
      echo "OK"
      break
    else
      echo -n "."
